...
I'm giving ranges for estimates because I don't want to charge for doing the research necessary to get a more accurate estimate.  Once you've chosen the items you want to implement, I'll spend time on each of them putting together a more accurate estimate so we can agree to a final cost for all.
...
### 2.1 Multi-Tenancy (Issue #18): 40-60 hours
The most foundational architectural change. Without it, two clients share the same data boundaries.  This ensures data security between clients and is critical for HIPAA/PHI support.
...
- Move all data access behind server-side API routes
...
### 2.3 Proper Identity Provider — Replace Custom Login: 40-80 hours
...
### 2.4 Azure Infrastructure & Proper CI/CD: 30-50 hours
...
### 2.4.1 Explicit hosted environments: 10-20 hours
...
- Each environment should have separate auth config, infrastructure, database, secrets, and deploy cadence

### 2.5 Schema Release Pipeline Hardening (Issue #15): 10-20 hours
Pre-requisite for safe production database operations with two clients.  Ensures we don't accidentally delete data.
...
### 2.6 Architecture Quality & Extensibility Pass: 25-45 hours
...
- Integrating Terra the first time will be expensive, but onboarding other devices will be much simpler after.  You only pay to integrate Terra once.
...
### 3.1 SRE / Observability Foundation: 20-40 hours
...
- On-call runbook and alerting to VOI team for P1s

### 3.2 Pulse Survey v2 (Issue #21): 5-15 hours
Complete spec exists.
- v2 question set with HRV/RHR context at submission time
- Aggregate dashboard updates for new questions

### 3.3 Performance at 175–200 Users: Database & Query Optimization: 10-20 hours
...
### 3.4 Dashboard Changes for a Larger Participant Group: 30-50 hours
...
### 3.5 Large-Cohort Test Data & UX Validation (User Acceptance Testing): 15-30 hours
...
### 3.6 User Journey Mapping & Full UX Review: 15-25 hours
Thoughtfully review each user type, their goals, and how they map to the system and what functionality they need.  Map the end-to-end journey for participant, wellness director, executive sponsor, and operator across every major page so UX changes are driven by actual workflow friction instead of isolated page fixes.
...
### 3.7 Mobile-Responsive Web UI: 30-60 hours
...
### 4.1 Anonymized Benchmark Comparisons (Issue #17): 15-30 hours
...
### 4.2 Intervention Tracking Logging (Issue #52): 4 hours
...
### 4.3 Mobile-Native UI: 100-200 hours
...
## 6. Appendix A: API Documentation

### 6.1 What the current import pipeline does (and what must change)
...
### 6.2 Application changes required for API ingestion

#### 6.2.1 OAuth token management infrastructure (new)
...
#### 6.2.2 New background sync worker (new — cannot run in Next.js API routes)
...
#### 6.2.3 New API-to-domain mappers (new)
...
#### 6.2.4 Data model changes: provider-agnostic schema
...
#### 6.2.5 Webhook ingestion endpoint (new, recommended for Fitbit)
...
#### 6.2.6 Import History and submission tracking rethink (Issue #74 dependency)
...
#### 6.2.7 Admin import UI remains needed (nice to have)
...
#### 6.2.8 Participant onboarding flow changes
...
### 6.3 WHOOP vs Fitbit: data richness comparison
...
### 6.4 Summary: what must be built for API ingestion
...
## 7. Appendix B: Personas & Key Journey Moments
...
### 7.1 The Four Personas

#### 7.1.1 Persona 1: The Participant
...
#### 7.1.2 Persona 2: The Wellness Director
...
#### 7.1.3 Persona 3: The Executive Sponsor / Buyer
...
#### 7.1.4 Persona 4: The VOILoop Operator
...
### 7.3 What this changes about prioritization
