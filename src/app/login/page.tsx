        ...
        <div style={{ background: '#002244', border: '1px solid #0a3560', borderRadius: 12, padding: 32 }}>
          {!sent ? (
            <>
              ...
              <p style={{ fontSize: 13, color: '#A5ACAF', marginBottom: 24, lineHeight: 1.5 }}>Enter your email. We will send you a secure link — no password needed.</p>
              <form onSubmit={handleLogin}>
                ...
                <button type="submit" disabled={loading || !email}
                  style={{ width: '100%', background: loading || !email ? '#0a3560' : '#69BE28', color: loading || !email ? '#A5ACAF' : '#002244', border: 'none', borderRadius: 8, padding: '12px', fontSize: 14, fontWeight: 700, cursor: loading || !email ? 'not-allowed' : 'pointer', fontFamily: 'Inter, sans-serif' }}>
                  {loading ? 'Sending...' : 'Send magic link →'}
                </button>
              </form>
              <div style={{ marginTop: 20, padding: '12px 14px', background: 'rgba(105,190,40,0.06)', border: '1px solid rgba(105,190,40,0.15)', borderRadius: 8, fontSize: 11, color: '#A5ACAF', lineHeight: 1.5 }}>
                🔒 <strong style={{ color: '#fff' }}>Secure access.</strong> Your link expires after 1 hour. Employee data is protected by role-based access control.
              </div>
            </>
          ) : (
            <div style={{ textAlign: 'center' }}>
              <div style={{ fontSize: 40, marginBottom: 16 }}>📬</div>
              <h2 style={{ fontSize: 18, fontWeight: 600, color: '#fff', marginBottom: 8 }}>Check your email</h2>
              <p style={{ fontSize: 13, color: '#A5ACAF', lineHeight: 1.6, marginBottom: 20 }}>We sent a magic link to <strong style={{ color: '#fff' }}>{email}</strong>. Click the link to sign in.</p>
              <div style={{ background: 'rgba(105,190,40,0.06)', border: '1px solid rgba(105,190,40,0.2)', borderRadius: 8, padding: '10px 14px', fontSize: 12, color: '#69BE28' }}>The link expires in 1 hour. Check your spam folder if needed.</div>
              <button onClick={() => { setSent(false); setEmail('') }}
                style={{ marginTop: 20, background: 'transparent', border: 'none', color: '#A5ACAF', fontSize: 12, cursor: 'pointer', fontFamily: 'Inter, sans-serif', textDecoration: 'underline' }}>
                Use a different email
              </button>
            </div>
          )}
        </div>
        <p style={{ textAlign: 'center', fontSize: 11, color: '#A5ACAF', marginTop: 20, lineHeight: 1.5 }}>VOILoop · Access controlled by your organization administrator.</p>
      </div>
    </div>
  )
}
